# -*- coding: utf-8 -*-
"""TB PIK FINAL DIRMA TERAKHIR.ipynb
...
DEFAULT_SOAL = [
    ...
    {
        "pertanyaan": "5. Hasil dari 12 × 12 + 24 ÷ 2 adalah?",
        "pilihan": ["168", "144", "156", "150", "160"],
        "jawaban_index": 2
    },
    {
        "pertanyaan": "6. Unsur kimia dengan simbol ‘Fe’ adalah?",
    ...
    {
        "pertanyaan": "13. Hasil dari √225 adalah?",
...
# MENU TAMBAH SOAL
        ...
        if menu == "Tambah Soal":
            st.header("➕ Tambah Soal Baru")

            pertanyaan = st.text_area("Masukkan Pertanyaan:")
            opsi1 = st.text_input("Pilihan 1:")
            opsi2 = st.text_input("Pilihan 2:")
            opsi3 = st.text_input("Pilihan 3:")
            opsi4 = st.text_input("Pilihan 4:")
            opsi5 = st.text_input("Pilihan 5:")

            jawaban_benar = st.selectbox(
                "Pilih jawaban yang benar:",
                ["Pilihan 1", "Pilihan 2", "Pilihan 3", "Pilihan 4", "Pilihan 5"]
            )

            if st.button("Simpan Soal"):
                if not pertanyaan or not opsi1 or not opsi2 or not opsi3 or not opsi4 or not opsi5:
                    st.error("Semua kolom harus diisi!")
                else:
                    new_question = {
                        "pertanyaan": pertanyaan,
                        "pilihan": [opsi1, opsi2, opsi3, opsi4, opsi5],
                        "jawaban_index": ["Pilihan 1", "Pilihan 2", "Pilihan 3", "Pilihan 4", "Pilihan 5"].index(jawaban_benar)
                    }

                    soal_data.append(new_question)
...
# MENU RIWAYAT
        if menu == "Riwayat Nilai & Jawaban":
            st.header("📄 Riwayat Nilai & Jawaban Peserta")

            if not skor_data and not jawaban_data:
                st.info("Belum ada data.")
            else:
                st.subheader("📌 Riwayat Nilai Peserta")
                if skor_data:
                    for i, e in enumerate(skor_data, start=1):
                        st.write(
                            f"**{i}.** {e['timestamp']} — **{e['nama']}** "
                            f"— Nilai: **{e['skor']}** ({e['benar']}/{e['total']})"
                        )
                else:
                ...
                st.subheader("📌 Riwayat Jawaban Peserta")
                if jawaban_data:
                    for data_user in jawaban_data:
                        st.markdown(f"### 👤 {data_user['nama']}  \n⏱️ *{data_user['timestamp']}*")
